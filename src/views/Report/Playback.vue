<template>
  <RouteTableModal :isOpen="isRouteModalPops" @close="routeModalToggle()" title="Route Details"
    :tableData="tableData"
    :startTime="startTime" 
    :startDate="startDate" 
    :endDate="endDate"
    :endTime="endTime"/>
  <EventDetailModal :isOpen="isEventModalPops" @close="eventModalToggle()" title="Event Details"
    :queryParams="eventParams" :imei="imei" />
  ...
  <div ref="mapContainer" class="map-container">
        ...
        <div class="flex flex-col gap-2">
          <div class="vehicle-info">
            <div class="flex items-center justify-between" @click="toggleAccordion()">
              ...
              <div>
                <button class="flex items-center space-x-3" :aria-expanded="isOpen" :aria-controls="`collapse`">
          ...
          <div class="violation-modal">
            <div class="flex flex-col gap-7">
              ...
              <select name="contractorFilter" id="contractorFilter"
                ...
                <option class="p-2 cursor-pointer" v-for="violation in violationFilterList" :value="violation.name">
    ...
    <div class="z-10 absolute left-2 bottom-2 bg-[#fefefe] rounded-lg p-4 flex gap-2">
      <div class="grid grid-cols-1 px-3 py-2 cursor-default gap-4 w-[400px]">
        <div class="flex gap-2 w-full text-left items-center" v-for="(item, index) in vehicle.devices">
          <input class="cursor-pointer w-4" type="checkbox" v-model="selectedVariant" :name="item.imeiNumber"
            :disabled="geoDataStore.status.isError" @change="hideRoute" :value="item.imeiNumber">
          <label :for="item.type" class="w-full items-center flex gap-4 font-semibold cursor-default">
          ...
          <div
            :class="{ 'blue-circle': index == 0, 'green-circle': index == 1, 'purple-circle': index == 2, 'red-circle': index == 3 }">
          </div>
          <div class="cursor-pointer shadow-lg p-1 rounded-full" @click="renderPoints(item.imeiNumber, index)"
            :class="{ 'disable-svg': isPlay[index] == true || layers[index] == undefined }">
            <svg v-if="!viewPoints[index]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
              fill="none">
              <path
          ...
          <div class="cursor-pointer shadow-lg p-1 rounded-full" @click="startPlayBack(item.imeiNumber, index)"
            :class="{ 'disable-svg': isPlay[index] == true || layers[index] == undefined }">
          ...
          <div class="cursor-pointer shadow-lg p-1 rounded-full"
            :class="{ 'disable-svg': isPlay[1 - index] || layers[index] == undefined || isStartPlayBack[index] == false }">
            <svg v-if="!isPlay[index]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
              fill="none" @click="playPause(index)">
          ...
          <div class="cursor-pointer shadow-lg p-2 rounded-md w-full text-center"
            :class="{ 'disable-svg': isPlay[index] == true || layers[index] == undefined }"
            @click="routeModalToggle(index)">
            <p>See Detail</p>
          </div>
        </div>
        <div class="flex flex-col gap-2 text-left">
          ...
          <div class="grid grid-cols-4 gap-4">
            <input class="col-span-3" type="range" name="playSpeed" v-model="playSpeed" min="10" max="1000" step="10"
              :disabled="isPlay[0] || isPlay[1]">
...
import RouteTableModal from '@/components/modal/RouteTableModal.vue'
...
let tableData
//Route Detail
const isRouteModalPops = ref(false)
function routeModalToggle(index) {
  if (index !== undefined) {
    tableData = geoDataStore.vehicleHistoryGeo.vehicle.devices[index].history
  }
  isRouteModalPops.value = !isRouteModalPops.value
}
...
.form-wrapper {
  @apply flex flex-col gap-4 items-end z-10 absolute right-2 top-2
}
...
.violation-modal {
  @apply shadow-sm shadow-blue-300/50 backdrop-blur-md bg-gradient-to-b from-slate-50/80 rounded-lg w-full px-4 py-6 gap-2 flex flex-col h-full min-w-[360px] cursor-pointer select-none
}

.vehicle-info {
  @apply shadow-sm shadow-blue-300/50 backdrop-blur-md bg-gradient-to-b from-slate-50/80 rounded-lg w-fit h-full px-4 py-6 gap-2 flex flex-col min-w-[360px] cursor-pointer select-none
}
