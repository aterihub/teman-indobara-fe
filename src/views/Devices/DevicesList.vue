  ...
  <div class="device-container">
    <div class="table-wrap">
      <div class="table-header">
        <h1 class="title"> Devices Management</h1>
...
import { storeToRefs } from 'pinia'
  ...
  const header = [
    { text: "", value: "indicator", width: 40, sortable: true},
    ...
    { text: "Vehicle Registration Number", value: "vehicleRegistrationNumber", sortable: true },
  ...
  async function getDevicesList() {
    ...
    mergedList.value = devicesList.value.map(device => {
      let tcpStatusData = devicesStatus.value.find(status => status.imei === device.IMEINumber) || defaultValue
      let vehicleName = device.vehicle.name
      let vehicleRegistrationNumber = device.vehicle.registrationNumber
      let variantName = device.variant.name
      return { ...device, ...tcpStatusData, vehicleName, vehicleRegistrationNumber, variantName}
    })
