<template>
  ...
  />
    ...
    <EditBrandModal 
      :isOpen="isEditBrandModalPops"
      @close="editModalToggle()"
      title="Edit Brand"
      :formData="items"/>
    <DeleteBrandModal
      :isOpen="isDelBrandModalPops"
      @close="isDelBrandModalPops = !isDelBrandModalPops"
      :deviceInfo="items"/>
      <div class="flex justify-between mb-6">
        <h1 class="title">Brand List</h1>
        <div class="button-wrapper">
          <Button type="button" class="outlined__green" label="Add New Brand" @click="newModalToggle()" />
        </div>
      </div>
...
import { storeToRefs } from 'pinia'
    ...
    const { getBrandIsLoading, getStatus } = storeToRefs(useVehicleBrandsStore())
    onBeforeMount(() => {
