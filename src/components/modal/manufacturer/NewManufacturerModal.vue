       ...
       <div class="modal" v-show="isOpen">
        <transition name="drop-in">
          <div class="modal-inner" v-show="isOpen" ref="target">
            ...
            <VeeForm :validation-schema="addManufacturerSchema" v-slot="{ handleSubmit }" as="div" ref="form" >
              <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
                <BaseInput name="name" type="text" placeholder="Define manufacturer name" class="outlined" label="Manufacturer Name"/>
...
import { addManufacturerSchema } from '@/composable/manufacturersSchema'
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    console.log(values)
    values.name = values.name.trim().toUpperCase()
    values.variant = values.variant.trim().toUpperCase()
    ...
    if (regButtonClick.value == 2) {
      await manufacturerStore.createManufacturer(values)
