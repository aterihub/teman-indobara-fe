          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            ...
            <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form" >
              <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
                <BaseInput name="name" type="text" placeholder="Define type name" class="outlined" label="Type Name"/>
                <div class="flex justify-between gap-10">
                  <BaseButton type="button" class="filled__softblue" :label="cancelLabel" @click="cancelForm"/>
                  <BaseButton type="submit" class="filled__green" :label="registerLabel" :loading="createTypeIsLoading"  />
                </div>
              </form>
            </VeeForm>
          </div>
...
import { useVehicleTypesStore } from '@/stores/master-data/vehicleTypesStore'
 ...
 const schema = yup.object({
  ...
  const { status, createTypeIsLoading } = storeToRefs(useVehicleTypesStore())
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (regButtonClick.value == 2) {
      ...
      await vehicleTypesStore.createVehicleType(values)
