     ...
     <div class="modal-inner" v-show="isOpen" ref="target">
       ...
       <VeeForm  v-slot="{ handleSubmit }" as="div" ref="form" >
         <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
          <BaseInput v-model="props.formData.name" name="name" type="text" placeholder="Define vehicle brand" class="outlined" label="Vehicle Brand"/>
           <div class="flex justify-between gap-10">
             <BaseButton type="button" class="filled__softblue" :label="cancelLabel" @click="cancelForm"/>
             <BaseButton type="submit" class="filled__green" :label="registerLabel" :loading="updateBrandIsLoading"  />
           </div>
         </form>
       </VeeForm>
     </div>
...
import { useVehicleBrandsStore } from '@/stores/master-data/vehicleBrandsStore'
  ...
  const { status, updateBrandIsLoading } = storeToRefs(useVehicleBrandsStore())
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (regButtonClick.value == 2) {
      await vehicleBrandsStore.updateVehicleBrand(props.formData.id,values) 
