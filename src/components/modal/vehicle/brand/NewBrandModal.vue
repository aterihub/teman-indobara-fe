          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            ...
            <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form" >
              <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
                <BaseInput name="name" type="text" placeholder="Define brand name" class="outlined" label="Brand Name"/>
                <div class="flex justify-between gap-10">
                  <BaseButton type="button" class="filled__softblue" :label="cancelLabel" @click="cancelForm"/>
                  <BaseButton type="submit" class="filled__green" :label="registerLabel" :loading="createBrandIsLoading"  />
                </div>
              </form>
            </VeeForm>
          </div>
...
import { useVehicleBrandsStore } from '@/stores/master-data/vehicleBrandsStore'
 ...
 const schema = yup.object({
  ...
  const { status, createBrandIsLoading } = storeToRefs(useVehicleBrandsStore())
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (regButtonClick.value == 2) {
      ...
      await vehicleBrandsStore.createVehicleBrand(values)
