          ...
          <div class="modal-inner" v-show="isOpen" ref="target">
            <div class="modal-content">
              <div class="flex flex-col gap-4">
                <h1 class="text-lg">Please verify the variant before deleting it</h1>
                <div class="flex flex-col gap-2">
                  <h1 class="font-semibold">Variant Name: {{ props.deviceInfo.variant }}</h1>
                </div>
              </div>
              <div class="button-wrapper">
                <BaseButton type="button" class="filled__red" :label="delLabel" :loading="deleteVariantIsLoading" @click="onSubmit(props.deviceInfo.id)" />
...
import { onClickOutside } from '@vueuse/core'
  ...
  const { status, deleteVariantIsLoading } = storeToRefs(useManufacturersStore())
  ...
  const onSubmit = async (id) => {
    ...
    if (delButtonClick.value == 2) {
      await manufacturersStore.deleteVariant(id)
